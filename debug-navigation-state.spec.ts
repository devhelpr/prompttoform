...
test('Debug Navigation State', async ({ page }) => {
  ...
  console.log('Console messages after Back to Editor:');
  consoleMessages.forEach((msg, index) => {
    console.log(`${index}: ${msg}`);
  });

  // Look for MainAppPage messages
  const mainAppPageMessages = consoleMessages.filter((msg) =>
    msg.includes('MainAppPage:')
  );

  if (mainAppPageMessages.length > 0) {
    console.log('✅ MainAppPage messages found:');
    mainAppPageMessages.forEach((msg) => console.log(`  - ${msg}`));
  } else {
    console.log('❌ No MainAppPage messages found');
  }
  ...
  // Take a screenshot
  await page.screenshot({ path: 'debug-navigation-state.png' });
});

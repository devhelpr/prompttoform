...
test.describe('Debug Import Validation', () => {
  test('should debug why Import JSON validation fails', async ({ page }) => {
    ...
    // Use the exact JSON you provided
    const bmiForm = {
      app: {
    ...
    console.log('Dialog content preview:', dialogText?.substring(0, 500));

    // Check for any error indicators
    const errorElements = page.locator('text=/error|invalid|required|failed/i');
    const errorCount = await errorElements.count();
    console.log('Error elements found:', errorCount);

    // Take a screenshot for debugging
    await page.screenshot({ path: 'debug-import-validation.png' });

    // The test should pass regardless of the validation result

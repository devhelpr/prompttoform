...
test('Field Property Sync Test', async ({ page }) => {
  // Navigate to the app
  ...
  // Find the sidebar and modify a field property
  ...
  // Check if the modified field label appears in the form preview
  ...
  const modifiedLabelFound = formPreviewContent?.includes(
  ...
  // Take a screenshot
  await page.screenshot({ path: 'test-field-property-sync.png' });

  // Log the results for debugging
  console.log('=== FIELD PROPERTY SYNC TEST RESULTS ===');
  console.log('✅ Page title sync working: true (from previous test)');
  console.log('⚠️ Field property sync working:', modifiedLabelFound);
  console.log('🔍 ISSUE: Field properties are not syncing properly');

  // The test should pass if we can at least see the sync mechanism working
  expect(modifiedLabelFound).toBe(true);
});
